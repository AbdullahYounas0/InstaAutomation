...
const WarmupPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    schedulerDelay: 0,
    visualMode: true,  // Default to true for consistency with other scripts
    activities: {
      feedScroll: true,
      watchReels: true,
      likeReels: true,
      ...
      <div className="script-content">
        <div className="script-form-section">
          <form onSubmit={handleSubmit} className="script-form">
            ...
            <div className="form-group">
              <label className="checkbox-item visual-mode-toggle">
                <input
                  type="checkbox"
                  checked={formData.visualMode}
                  onChange={(e) => setFormData({...formData, visualMode: e.target.checked})}
                  disabled={isRunning}
                />
                <span>
                  <strong>Visual Mode</strong>
                  <small style={{display: 'block', color: '#666', marginTop: '4px'}}>
                    Show browser windows (useful for debugging, but uses more resources)
                  </small>
                </span>
              </label>
            </div>
